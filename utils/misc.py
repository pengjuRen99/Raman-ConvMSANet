...
class EarlyStopping(object):
    def __init__(self, monitor: str = 'val_acc1', mode: str = 'max', patience: int = 1):
        '''
            monitor: 要监测的指标，需要输入字典
            mode: 监测指标模式，min/max
            patience: 最大容忍次数
        '''
        # example
    ...
    def __call__(self, metrics) -> bool:
        ...
        if (self.mode == 'min' and metrics <= self.__value) or (self.mode == 'max' and metrics >= self.__value):
