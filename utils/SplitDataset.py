...
class SplitDataset():
    ...
    def __get_label_names(self):                
        ...
        label_names.sort()
        class_indices = dict((k, v) for v, k in enumerate(label_names))
        json_str = json.dumps(dict((val, key) for key, val in class_indices.items()), indent=4, ensure_ascii=False)
        with open('class_Transmission.json', 'w') as json_file:
    ...
    def __copy_files(self, type_path, type_saved_dir):                  # copy files 
    ...
    def __split_dataset(self, i):                                        
        ...
        for index in range(len(all_file_paths)):
            ...
            train_num = int(file_path_list_length * self.train_ratio)
            test_num = int(file_path_list_length * self.test_ratio)
            self.test_file_path.append([self.index_label_dict[index], file_path_list[i*test_num: (i+1)*test_num]])
            if i == 0:
                self.train_file_path.append([self.index_label_dict[index], file_path_list[(i+1)*test_num: (i+1)*test_num+train_num]])
                self.valid_file_path.append([self.index_label_dict[index], file_path_list[(i+1)*test_num+train_num :]])
            elif i == 9:
                self.train_file_path.append([self.index_label_dict[index], file_path_list[0: train_num]])
                self.valid_file_path.append([self.index_label_dict[index], file_path_list[train_num: i*test_num]])
            else:
                self.train_file_path.append([self.index_label_dict[index], file_path_list[0: i*test_num]])
                self.train_file_path.append([self.index_label_dict[index], file_path_list[(i+1)*test_num: (i+1)*test_num+train_num-i*test_num]])  
                self.valid_file_path.append([self.index_label_dict[index], file_path_list[(i+1)*test_num+train_num-i*test_num :]])


    def start_splitting(self, i):
        self.__split_dataset(i=i)
...
def cross_validation(i):            
    split_dataset = SplitDataset(dataset_dir='animal_blood_data/Transmissive_animalBlood_preprocessing', saved_dataset_dir='split_animal_data/Transmissive_blood_dataset', show_progress=True)
    split_dataset.start_splitting(i=i)
